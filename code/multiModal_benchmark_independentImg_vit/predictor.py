...
class Simulation(object):
    ...
    def __init__(self, cfg):
        ...
        self.data = self.dataloader.inferLoadData()
    ...
    def run(self):
        ...
        for phase in range(self.args["rerun"]):
            ...
            model = MultiModalWLT(self.args, tokenizer_len=len(self.data["test"].dataset.tokenizer))
            model.load_state_dict(torch.load(modelPath))
            predictor = Predictor(self.args)
            pred = predictor.predict(model, self.data["test"])
